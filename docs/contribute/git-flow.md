...
```bash
git remote add upstream  https://github.com/hadenlabs/ansible-role-nginx.git
```

Now your **upstream** points to **hadenlabs/ansible-role-nginx**.
...
git fetch upstream # assuming upstream is hadenlabs/ansible-role-nginx
...
All merge requests are automatically tested using [Github Actions](https://github.com/hadenlabs/ansible-role-nginx/actions). In case some tests fail, fix the issues or describe why the fix cannot be done.
